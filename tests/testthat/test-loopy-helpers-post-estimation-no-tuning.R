...
test_that("post with estimation, no tuning, with submodels, classification", {
	skip_if_not_installed("modeldata")
	skip_if_not_installed("C50")

	cls <- make_post_data()

	wflow <- workflow(class ~ ., knn_cls_spec, cls_cal)
...
test_that("post with estimation, no tuning, with submodels, regression", {
	skip_if_not_installed("kernlab")
	skip_if_not_installed("modeldata")
