test_that("stage helpers - recipe only", {
	...
	expect_equal(dim(melodie:::remove_stage(sched_1)), c(3L, 1L))
	expect_s3_class(
		sched_1,
		c("grid_schedule", "schedule", "tbl_df", "tbl", "data.frame")
	)

	expect_true(melodie:::has_pre_param(sched_1))
	expect_false(melodie:::has_sub_param(sched_1$model_stage[[1]]$predict_stage))
})
...
test_that("stage helpers - model only", {
	...
	expect_false(melodie:::has_pre_param(sched_1))
	expect_true(melodie:::has_mod_param(sched_1))
	expect_equal(
		purrr:::map_lgl(
			sched_1$model_stage[[1]]$predict_stage,
			melodie:::has_sub_param
		),
		rep(TRUE, 3)
	)
	expect_equal(
	...
	expect_false(melodie:::has_pre_param(sched_2))
	expect_true(melodie:::has_mod_param(sched_2))
	expect_equal(
		purrr:::map_lgl(
...
test_that("stage helpers - post only", {
	...
	expect_false(melodie:::has_pre_param(sched_1))
	expect_false(melodie:::has_mod_param(sched_1$model_stage[[1]]))
})
...
test_that("stage helpers - recipe and model", {
	...
	expect_equal(
		purrr::map_lgl(sched_1$model_stage, melodie:::has_mod_param),
		rep(TRUE, 3)
	)
	...
	expect_equal(
		purrr::map_lgl(
			sched_1$model_stage,
			~melodie:::has_sub_param(.x$predict_stage)
		),
		rep(FALSE, 3)
	)
	...
	expect_equal(dim(melodie:::remove_stage(sched_2)), c(3L, 1L))
	expect_s3_class(
		sched_2,
		c("grid_schedule", "schedule", "tbl_df", "tbl", "data.frame")
	)

	expect_true(melodie:::has_pre_param(sched_2))

	expect_equal(
		purrr::map_lgl(sched_2$model_stage, melodie:::has_mod_param),
		rep(TRUE, 3)
	)
	...
	expect_equal(
		purrr::map_lgl(
			sched_2$model_stage,
			~melodie:::has_sub_param(.x$predict_stage[[1]])
		),
		rep(TRUE, 3)
	)
})
