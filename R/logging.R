...
tune_catalog <- function(issues) {
	...
	rlang::env_bind(
		...
		catalog_summary = summarize_catalog(res)
	)
...
.catch_and_log_fit <- function(.expr, ..., notes) {
	...
	if (!is_workflow(result)) {
		cli::cli_abort(
			"Internal error: Model result is not a workflow, but
